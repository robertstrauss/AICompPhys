...
u0 = Float32[π*0.8; 0.]  # angle, angular velocity
g_over_l = 9.8
# simulation resolution and time span
datasize = 100
tspan = (0.0,sqrt(g_over_l)*6)

function eye(n,m)
     foo = zeros( Float32,m,n )
     for i in 1:min(n,m)
       foo[i,i]=1.0f0
     end
     foo
...
# resnet skip netlwoirk
#https://discourse.julialang.org/t/best-way-to-implement-shortcut-connections-for-feed-forward-neural-networks-in-flux-jl/10633
...
function QuadLayer( c::Real )
  QuadLayer(Flux.param(c))
end
function (a::QuadLayer)(x::AbstractArray)
    a.c.*x.*x
    end

Flux.@treelike QuadLayer

struct CubeLayer
  c
end

function CubeLayer( c::Real )
  CubeLayer(Flux.param(c))
end
function (a::CubeLayer)(x::AbstractArray)
    a.c.*x.*x.*x
    end

Flux.@treelike CubeLayer
...
function sinhtanh(W::AbstractArray)
    sinhtanh(Flux.param(W))
end

function (a:sinhtanh)(x::AbstractArray)
  u = a.W*x
  u[1:2:end] = sinh.(1:2:end)
  u[2:2:end] =  tanh.(2:2:end)
  return u
end
...
struct myDense2
  W
  b
  beta  # need union of Array and Tracked Arrya
  activation::Function
  # myDense2(W, b, beta, activation) = new(Flux.param(W), )
end

function myDense2(activation::Function,W, b, beta::Real)
    myDense2(Flux.param(W),Flux.param(b),Flux.param([beta]),activation)# cast the array to a tracked parameter list
                            # then create and return the struct
end
function myDense2(activation::Function, W, beta::Real )
    myDense2(Flux.param(W),zeros(Float32,size(W)[1]),Flux.param([beta]),activation )# cast the array to a tracked parameter list
                            # then create and return the struct
end
function myDense2(W)
    myDense2(Flux.param(W),zeros(Float32,size(W)[1]),[0.0f0],identity)# cast the array to a tracked parameter list
                            # then create and return the struct
end
...
function setupNet1()
  ...
  hh[2,2] = 0.0f0
  dudt = Chain(
     CatSkip(myDense2(tanh, eye(2,2), 1.0f0) ),

     myDense2(eye(4,4)),
     myDense2(hh),

  #  myDense2( hh, zeros(Float32,4), [0.0], identity,"m"),

    IdentitySkip( myDense2(tanh,zeros(Float32,4,4),0.0f0)),
    myDense2(eye(4,2)),
...
prob = ODEProblem(trueODEfunc0,u0,tspan)
...
# initial guess
pu = [0.0f0 1.0f0 0.0f0 0.0f0 ;  -9.7f0 0.0f0 0.0f0 0.0f0 ; 0.0f0 0.0f0 1.0f0 0.0f0; 0.0f0 0.0f0 0.0f0 1.0f0]  # I'm adding in this variation. A single param to Rackaukus's example
...
function TrainIt(Iterations::Int=3; opt = ADAM(0.1), cb = cb1, ps=ps, loss_n_ode=loss_n_ode)
  ...
  data = Iterators.repeated((), Iterations) # We don't care how well it trains, just test if training works, so 2 iterations

  Flux.train!(loss_n_ode, ps, data, opt, cb = cb)
end
#TrainIt(10,opt=ADAM(0.01))

TrainIt(10,opt=ADAM(0.01))
TrainIt(20,opt=ADAM(0.003))
TrainIt(50,opt=ADAM(0.001))
TrainIt(600,opt=ADAM(0.0006))
TrainIt(1000,opt=ADAM(0.0003))
TrainIt(3000,opt=ADAM(0.0001))
TrainIt()
TrainIt(100)
...
Stacktrace:
 ...
 [2] #gradient#24(::Bool, ::typeof(Tracker.gradient), ::Function, ::Tracker.Params) at /Users/cems/.julia/packages/Tracker/cpxco/src/back.jl:1
